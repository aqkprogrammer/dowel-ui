...
// Ported from SmoothUI Pricing 3 (MIT, © 2024 Eduardo Calvo). See THIRD_PARTY_NOTICES.md.
import {
  useId,
  useState,
...
/*
 * Two plans side by side — a free tier and a featured paid one — under a
 ...
 * so two "Get started" buttons are distinguishable out of context. The
...
const PREFIX = "dowel-pricing-two-tier";

const STYLES = `
@keyframes ${PREFIX}-rise{from{opacity:0;translate:0 2.5rem}}
[data-slot=pricing-two-tier-plans]>*{animation:${PREFIX}-rise var(--duration-slow) var(--ease-out-quint) both;animation-delay:calc(var(--${PREFIX}-index,0) * 100ms * var(--motion-scale,1))}
`;
...
export interface PricingTwoTierLabels {
...
const DEFAULT_LABELS: PricingTwoTierLabels = {
...
export const DEFAULT_PRICING_TWO_TIER_PLANS: PricingTierPlan[] = [
  {
    id: "hobby",
    name: "Hobby",
    tagline: "Perfect for personal projects",
    description:
      "Deploy personal projects and experiment with the latest web technologies. Ideal for learning and side projects.",
    monthly: 0,
    features: [
      "100 GB bandwidth",
    ...
    cta: { label: "Get started", href: "#hobby" },
  },
  {
    id: "pro",
    name: "Pro",
    tagline: "For production",
    description:
      "Advanced deployment features for production applications, for teams building and scaling professional web apps.",
    monthly: 25,
    yearly: 12,
    featured: true,
    features: [
...
export interface PricingTwoTierBlockProps extends Omit<
...
> {
  ...
  labels?: Partial<PricingTwoTierLabels>;
...
function PeriodControl({
  ...
  return (
    <RadioGroup
      ...
      data-slot="pricing-two-tier-period"
...
/** Two plans side by side under a monthly/annual switch, with prices that roll. */
export function PricingTwoTierBlock({
  heading = "Deploy faster with Pro",
  description = "Scale your applications with advanced deployment features and priority support.",
  plans = DEFAULT_PRICING_TWO_TIER_PLANS,
  currency = "EUR",
  locale,
  period: periodProp,
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        ...
        <ul
          data-slot="pricing-two-tier-plans"
